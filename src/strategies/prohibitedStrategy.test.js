...
import { WorkflowStrategyFactory } from '../factories/workflowStrategyFactory';
...
describe('ProhibitedStrategy', () => {
  ...
  it('should handle prohibitionCheckAtHostRefCountryLevel', async () => {
    ...
    let searchInputMock = { plantDetails: { country: 'testCountry', serviceFormat: 'testFormat', hostRef: '123' } };
    ...
    //   });
      ...
      await new workflowEngine( plantDocumentMock,
        plantNameDocMock,
        searchInputMock,
        countryMappingMock,
        loggerMock)
    ...
    const result = await prohibitedStrategy.execute();
    expect(loggerMock?.info).toHaveBeenCalledWith('Starting Prohibited check at HOST_REF, COUNTRY level');
    console.log(result.loggerMock.info)
    expect(result.annexSixRule).toBe('testRule');
    expect(result.loggerMock?.info).toHaveBeenCalledWith('Starting Prohibited check at HOST_REF, COUNTRY level');
    expect(result.loggerMock?.info).toHaveBeenCalledWith('Annex6 (PROHIBITED) rule is APPLICABLE at HOST_REF, COUNTRY level, testRule, testCountry, testCountry, testFormat');
  });
