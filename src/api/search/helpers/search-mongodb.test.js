import {
  ...
  searchPestDetailsDb,
  getCountries
} from '~/src/api/search/helpers/search-mongodb'
...
describe('searchPlantDetailsDb', () => {
  ...
  it('should return plant details for LATIN_NAME, COMMON_NAME, and SYNONYM_NAME', async () => {
    ...
    expect(result).toEqual([
      {
        id: 'latin-name',
        results: [{ plantName: 'Rose', hostRef: '123', eppoCode: '456' }]
      },
      {
        id: 'common-name',
        results: [{ plantName: 'Rose', hostRef: '123', eppoCode: '456' }]
  ...
  it('should return an error message if the search fails', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith(
      `Search query failed Error: ${errorMessage}`
    )
    expect(result).toBe(errorMessage)
  })
  ...
  it('should return an error message if the country query fails', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith(
      `Countries could not be fetched Error: ${errorMessage}`
    )
    expect(result).toBe(errorMessage)
  })
})
