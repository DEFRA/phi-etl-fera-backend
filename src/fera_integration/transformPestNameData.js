...
export const transformPestData = async (data) => {
  ...
  const transformedData = await Promise.all(data.map(async apiItem => {
    const apiValidated = pestApiSchema.load(apiItem).validate().getProperties()
    const mongoItem = {}
 
    for (const [apiField, mongoField] of Object.entries(apiToMongoFieldMap)) {
      if (apiField === 'ArrayPestCommonNames') {
        mongoItem[mongoField] = {
          COMMON_NAME_ID: (apiValidated[apiField] || []).map(common => common.PestCommonNameId || null),
          COMMON_NAME: (apiValidated[apiField] || []).map(common => common.PestCommonName || "")
        }
      } else if (apiField === 'ArrayPestSynonyms') {
        mongoItem[mongoField] = {
          SYNONYM_NAME_ID: (apiValidated[apiField] || []).map(syn => syn.SynonymId || null),
          SYNONYM_NAME: (apiValidated[apiField] || []).map(syn => syn.SynonymName || "")
        }
      } else {
  ...
  const outputPath = path.resolve('C:/Projects/DEFRA/PHI-Beta/FERA Data/API Response/OneDrive_2024-10-31/FERA APIs-3110/PestData-transformed.json')
