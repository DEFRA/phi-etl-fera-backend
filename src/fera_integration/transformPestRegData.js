...
export const transformPestRegulationData = async (data) => {
  const transformedData = await Promise.all(data.flatMap(apiItem => {
    const apiValidated = pestRegulationApiSchema.load(apiItem).validate().getProperties()
    ...
    apiValidated.ArrayRegulation.forEach(regulation => {
      regulation.ArrayRegulationHost.forEach(host => {
        ...
        mongoItem[apiToMongoFieldMap.ArrayRegulation.Regulation] = regulation.Regulation
        mongoItem[apiToMongoFieldMap.ArrayRegulation.RegulationCategory] = regulation.RegulationCategory
        mongoItem[apiToMongoFieldMap.ArrayRegulation.RegulationCategoryDescription] = regulation.RegulationCategoryDescription || ''
 
        mongoItem[apiToMongoFieldMap.ArrayRegulation.ArrayRegulationHost.HostRef] = host.HostRef || null
        mongoItem[apiToMongoFieldMap.ArrayRegulation.ArrayRegulationHost.HostFormatId] = host.HostFormatId || null
        mongoItem[apiToMongoFieldMap.ArrayRegulation.ArrayRegulationHost.HostFormat] = host.HostFormat || ''
        ...
        const validatedMongoItem = pestRegulationMongoSchema.load(mongoItem).validate().getProperties()
  ...
  const outputPath = path.resolve('C:/Projects/DEFRA/PHI-Beta/FERA Data/API Response/OneDrive_2024-10-31/FERA APIs-3110/PestRegulationData-transformed.json')
