    ...
    for (const [apiField, mongoField] of Object.entries(apiToMongoFieldMap)) {
      if (apiField === "TaxonomicLevel (F/G/S)") {
        ...
        mongoItem[mongoField] = {
          ID: (apiValidated[apiField] || []).map(common => common.CommonNameId || null),
          NAME: (apiValidated[apiField] || []).map(common => common.CommonName || "")
        }
      } else if (apiField === 'ArraySynonyms') {
        mongoItem[mongoField] = {
          ID: (apiValidated[apiField] || []).map(syn => syn.SynonymId || null),
          NAME: (apiValidated[apiField] || []).map(syn => syn.SynonymName || "")
        }
      } else {
